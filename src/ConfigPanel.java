...
public class ConfigPanel extends JPanel implements ChangeListener, KeyListener {
	 ...
	 * Field pilotSpinner.
	 */
	private final JSpinner pilotSpinner, fighterSpinner, traderSpinner,
			engineerSpinner;
	...
	public ConfigPanel(ActionListener listener) {
		 ...
		 */
		// a rigid area in the south panel that can be adjusted to control the
	...
	/**
	 * When activated, this method creates the player from the input fields. The
	...
	public void stateChanged(ChangeEvent e) {
		 ...
		 * skill points and reset the maximum bounds on the spinner values.
		 */
		if (e.getSource().equals(pilotSpinner)
			...
			remainingPoints = NUM_SKILL_POINTS - (Integer) pilotSpinner.getValue()
					...
					- (Integer) traderSpinner.getValue()
					- (Integer) engineerSpinner.getValue();
			pilotSpinner.setModel(new SpinnerNumberModel((Number) pilotSpinner
					.getValue(), (Integer) 0, remainingPoints
					+ (Integer) pilotSpinner.getValue(), (Number) 1));
			fighterSpinner.setModel(new SpinnerNumberModel(
					(Number) fighterSpinner.getValue(), (Integer) 0,
					remainingPoints + (Integer) fighterSpinner.getValue(),
					(Number) 1));
			traderSpinner.setModel(new SpinnerNumberModel(
					(Number) traderSpinner.getValue(), (Integer) 0,
					remainingPoints + (Integer) traderSpinner.getValue(),
					(Number) 1));
			engineerSpinner.setModel(new SpinnerNumberModel(
					(Number) engineerSpinner.getValue(), (Integer) 0,
					remainingPoints + (Integer) engineerSpinner.getValue(),
					(Number) 1));
		...
		if (remainingPoints == 0
				...
				&& (e.getSource().equals(pilotSpinner)
						|| e.getSource().equals(fighterSpinner)
						|| e.getSource().equals(traderSpinner) || e.getSource()
						.equals(engineerSpinner))) {
